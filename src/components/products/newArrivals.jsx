...
const NewArrivals = () => {
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const { data } = await axios.get(`${URL}/user/latestProducts`, config);
  ...
  const handleViewAll = () => {
    // Navigate to shop page with filter for new arrival products
    navigate('/shop?isLatestProduct=true&sort=created-desc');
  };
...
{(isLoading || (!error && products.length > 0)) && (
  <div className='flex justify-between items-center w-[98%] md:w-11/12 mx-auto mb-8'>
    ...
    {!isLoading && products.length > 0 && (
      ...
      {!isLoading && products.length > 0 && (
        <div className="new-arrivals-pagination flex justify-center space-x-2 mt-4"></div>
      )}
    </div>
  );
};
